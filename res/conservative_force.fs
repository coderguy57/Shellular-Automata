...
//	Output Values
	...
	if(stage == GET_DEMAND) {
		...
		// float conv1 = nh_rings_c[1].value.b;
		// res_c.b = ((res_c.r + (conv1 * (s*2.13333))) / (1.0 + (s*2.13333))) - 0.01 * s;
		// res_c.b += res_c.r * 0.01;
		// res_c.b = min(1., res_c.b);
		// float conv = nh_rings_c[1].value.b;
		// float size = nh_rings_c[0].total;
		// res_c.b += conv / size * 0.01;
		// res_c.b -= 0.01 * 0.01;
		// res_c.b -= size * 0.01;
		float target = 0.5;
		target += 0.5 * res_c.r;
		...
		// demand.b = (0.5 - abs(res_c.b - target)) / area;
		// demand.b = abs(res_c.b - target) / area;
		// demand.b = 1. / area;
		// demand.b = 0. / area;
		// demand.b = (1. - res_c.b) / area;
